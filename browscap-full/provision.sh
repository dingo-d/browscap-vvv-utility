...
if [[ -s "/etc/php/${PHP_VER}/mods-available/full_php_browscap.ini" ]]; then
  echo "Full Browscap is already installed or is empty"
  copy_browscap
else
  copy_browscap
fi
...
if [[ -f "/etc/php/${PHP_VER}/fpm/php.ini" ]]; then
  ...
  if grep -q ";browscap" "/etc/php/${PHP_VER}/fpm/php.ini"; then
    sudo sed -i "s|;browscap =.*|browscap = /etc/php/${PHP_VER}/mods-available/full_php_browscap.ini|g" "/etc/php/${PHP_VER}/fpm/php.ini"
  else
    sudo sed -i "s|browscap =.*|browscap = /etc/php/${PHP_VER}/mods-available/full_php_browscap.ini|g" "/etc/php/${PHP_VER}/fpm/php.ini"
  fi
